...
import static net.dermetfan.gdx.math.MathUtils.*;
import static net.dermetfan.gdx.physics.box2d.Box2DUtils.Settings.*;
...
public class Box2DUtils {
    ...
    public enum PreconditionCheck {
        ...
        public static class InvalidPolygonShapeException extends InvalidPolyShapeException {
            ...
            public InvalidPolygonShapeException(String message, Problem problem, float[] vertices, int offset, int
                    length) {
        ...
        public static class InvalidChainShapeException extends InvalidPolyShapeException {
            ...
            public enum Problem {
                ...
                /** too close vertices: the squared distance between at least 2 vertices is closer than {@link
                 * Settings#linearSlop} squared */
                CLOSE_VERTICES
            ...
            public InvalidChainShapeException(String message, Problem problem, float[] vertices, int offset, int
                    length) {
                super(message, vertices, offset, length);
                this.problem = problem;
            }
        ...
        public static void checkChainShape(float[] vertices, int offset, int length) {
            ...
            if (length % 2 != 0) {
                throw new InvalidChainShapeException("chain vertices are malformed. vertices.length: " + length,
                        InvalidChainShapeException.Problem.MALFORMED_VERTICES, vertices, offset, length);
            }
            if (length < 4) {
                throw new InvalidChainShapeException("chain has less than 2 vertices: vertices.length: " + length,
                        InvalidChainShapeException.Problem.VERTEX_COUNT, vertices, offset, length);
            }
            ...
            if (verticesTooClose) {
                throw new InvalidChainShapeException("chain vertices are too close together",
                        InvalidChainShapeException.Problem.CLOSE_VERTICES, vertices, offset, length);
            }
        }
        ...
        public static void checkPolygonShape(float[] vertices, int offset, int length) {
            ArrayUtils.checkRegion(vertices, offset, length);
            if (length % 2 != 0) {
                throw new InvalidPolygonShapeException("polygon vertices are malformed. vertices.length: " + length,
                        InvalidPolygonShapeException.Problem.MALFORMED_VERTICES, vertices, offset, length);
            }
            if (length < 6 || length > maxPolygonVertices * 2) {
                throw new InvalidPolygonShapeException("polygon has invalid number of vertices (min: 3, max: Settings" +
                        ".maxPolygonVertices = " + maxPolygonVertices + "). length: " + length,
                        InvalidPolygonShapeException.Problem.VERTEX_COUNT, vertices, offset, length);
            }
            float[] floats = GeometryUtils.getFloats();
            System.arraycopy(vertices, offset, floats, 0, length);
            int count = weld(floats, 0, length);
            if (count < 3) {
                throw new InvalidPolygonShapeException("polygon has too few vertices after welding: " + count,
                        InvalidPolygonShapeException.Problem.VERTEX_COUNT, vertices, offset, length);
            }
            if (!GeometryUtils.isConvex(vertices, offset, length)) {
                throw new InvalidPolygonShapeException("polygon is concave", InvalidPolygonShapeException.Problem
                        .CONCAVE, vertices, offset, length);
            }
            float area = GeometryUtils.polygonArea(vertices, offset, length);
            if (area < epsilon) {
                throw new InvalidPolygonShapeException("polygon area is too small: " + area + " (min is Settings" +
                        ".epsilon: " + epsilon + ")", InvalidPolygonShapeException.Problem.AREA, vertices, offset,
                        length);
            }
        }
    ...
    /** Cached {@link Shape Shapes} and their {@link ShapeCache}. You should {@link ObjectMap#clear() clear} this
     * when you don't use the shapes anymore. */
    ...
    public static ShapeCache cache(Shape shape) {
        ...
        ShapeCache results = new ShapeCache(cachedVertices, width0(shape), height0(shape), minX0(shape), maxX0(shape)
                , minY0(shape), maxY0(shape));
    ...
    private static float[] vertices0(Shape shape) {
        ...
        switch (shape.getType()) {
            ...
            case Circle:
                ...
                break;
            default:
                throw new IllegalArgumentException("shapes of the type '" + shape.getType().name() + "' are not " +
                        "supported");
        }
        return vertices;
    }
    ...
    public static Vector2 position(Shape shape, Body body) {
        return body.getPosition().add(positionRelative(shape, body.getAngle() * com.badlogic.gdx.math.MathUtils
                .radDeg));
    }

    /** @see #positionRelative(Shape, float) */
    public static Vector2 positionRelative(Fixture fixture) {
        return positionRelative(fixture.getShape(), fixture.getBody().getAngle() * com.badlogic.gdx.math.MathUtils
                .radDeg);
    }
    ...
    public static Rectangle aabb(Shape shape, float rotation, Rectangle aabb) {
        ...
        // if polygonVertices.length > vertices.length, set remaining polygonVertices to the last vertex from
        // vertices to make them redundant
        ...
        if (shape.getType() == Type.Circle) {
            polygon.setOrigin(GeometryUtils.minX(vertices) + GeometryUtils.width(vertices) / 2, GeometryUtils.minY
                    (vertices) + GeometryUtils.height(vertices) / 2);
    ...
    public static Rectangle aabb(Fixture fixture, Rectangle aabb) {
        return aabb(fixture.getShape(), fixture.getBody().getAngle(), aabb).setPosition(fixture.getBody().getPosition
                ().add(aabb.x, aabb.y));
    }
    ...
    /** @return the given Rectangle set as axis aligned bounding box of all fixtures of the given Body, in world
     * coordinates
     *  @since 0.9.1 */
    public static Rectangle aabb(Body body, Rectangle aabb) {
        float minX = Float.POSITIVE_INFINITY, minY = Float.POSITIVE_INFINITY, maxX = Float.NEGATIVE_INFINITY, maxY =
     ...
     *  @param shape if the {@link Fixture#getShape() shape} of the given Fixture should be deep {@link #clone(Shape)
     *  copied} as well
    ...
    public static Fixture clone(Fixture fixture, Body body, boolean shape) {
        ...
        clone.setUserData(fixture.getUserData());
        return clone;
    }
     ...
     *  @return If the body was successfully split, which means that all fixtures intersecting with the given segment
     *  were split. If false, only some fixtures may have been created! */
     ...
     *  @param a the first point of the segment
     *  @param b the second point of the segment
     *  @param aBody The body the first resulting fixture will be created on. No fixture will be created if this is
     *               null.
     *  @param bBody The body the second resulting fixture will be created on. No fixture will be created if this is
     *               null.
    ...
    public static boolean split(Fixture fixture, Vector2 a, Vector2 b, Body aBody, Body bBody, Pair<Fixture, Fixture>
            store) {
        @SuppressWarnings("unchecked")
        ...
        Fixture aFixture = aBody != null && defs.hasKey() ? aBody.createFixture(defs.getKey()) : null, bFixture =
                bBody != null && defs.hasValue() ? bBody.createFixture(defs.getValue()) : null;
        if (defs.hasKey()) {
    ...
    public static boolean split(Fixture fixture, Vector2 a, Vector2 b, Pair<FixtureDef, FixtureDef> store) {
        ...
        Vector2 tmpA = Pools.obtain(Vector2.class).set(a).sub(bodyPos), tmpB = Pools.obtain(Vector2.class).set(b).sub
                (bodyPos);
    ...
    public static <T extends Shape> boolean split(T shape, Vector2 a, Vector2 b, Pair<T, T> store) {
        ...
        if (type == Type.Circle) {
            throw new IllegalArgumentException("shapes of the type " + Type.Circle + " cannot be split since Box2D " +
                    "does not support curved shapes other than circles: " + shape);
        }

        if (type == Type.Edge) {
            Vector2 vertex1 = Pools.obtain(Vector2.class), vertex2 = Pools.obtain(Vector2.class), intersection =
                    Pools.obtain(Vector2.class);
        ...
        if (type == Type.Polygon) {
            ...
            for (int i = 1; i < vertices.length; i += 2) {
                ...
                if (!intersected && i + 2 < vertices.length && Intersector.intersectSegments(x, y, vertices[i + 1],
                        vertices[i + 2], a.x, a.y, b.x, b.y, tmp)) {
                    intersected = true;
                    aVertices.add(tmp.x);
                    aVertices.add(tmp.y);
                    bVertices.add(tmp.x);
    ...
    public static int weld(float[] vertices, int offset, int length) {
        ...
        for (int i = offset; i + 1 < offset + length; ) {
            float x1 = vertices[i], y1 = vertices[i + 1], x2 = vertices[ArrayUtils.repeat(offset, length, i + 2)], y2
                    = vertices[ArrayUtils.repeat(offset, length, i + 3)];
